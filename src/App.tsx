...
<button 
    ...
    >
              ...
              {/* Dropdown removed and moved to global modal */}
            </div>
    ...
    >
          ...
          <div className={`mb-8 sm:mb-12 grid grid-cols-1 gap-8 sm:gap-12 ${tripType === 'round-trip' ? 'lg:grid-cols-2 lg:gap-20' : ''}`}>
            <div className="min-w-0 relative">
              ...
              <div className="flex overflow-x-auto gap-4 pt-3 pb-6 px-1 soft-scrollbar scroll-smooth">
                {dates.map((d) => (
                  <button
                    key={d.id}
                    ...
                    className={`flex flex-col items-center justify-center min-w-[82px] sm:min-w-[100px] py-3 sm:py-4 px-4 sm:px-6 rounded-3xl transition-all duration-300 border ${
                      selectedDate === d.id
                        ? 'bg-slate-900 border-slate-900 dark:bg-white dark:border-white text-white dark:text-slate-900 shadow-[0_12px_25px_rgba(0,0,0,0.15)] scale-105 z-10'
                        : 'bg-slate-50 border-slate-100 dark:bg-slate-900/50 dark:border-slate-800 text-slate-500 hover:bg-slate-100 hover:border-slate-200 dark:hover:bg-slate-800 dark:hover:border-slate-700'
                    }`}
                  >
                    <span className={`text-[11px] font-bold mb-1.5 uppercase tracking-tighter ${selectedDate === d.id ? 'text-slate-400 dark:text-slate-500' : 'text-slate-400 dark:text-slate-500'}`}>
                      {d.label}
                    </span>
                    <span className={`text-xl font-black ${selectedDate === d.id ? 'text-white dark:text-slate-900' : 'text-slate-700 dark:text-slate-300'}`}>
                      {d.date}
                    </span>
                  </button>
                ))}
                ...
                <div className="flex overflow-x-auto gap-4 pt-3 pb-6 px-1 soft-scrollbar scroll-smooth">
                  {dates.map((d) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-[11px] font-bold mb-1.5 uppercase tracking-tighter ${returnDate === d.id ? 'text-blue-200' : 'text-slate-400 dark:text-slate-500'}`}>
                          {d.label}
                        </span>
                        <span className={`text-xl font-black ${returnDate === d.id ? 'text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                          {d.date}
                        </span>
                      </button>
                    );
