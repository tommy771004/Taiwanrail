...
| :--- | :--- | :--- |
| `VITE_TDX_CLIENT_ID` | `您的 TDX Client ID` | 從 TDX 官網取得的應用程式 ID。 |
| `VITE_TDX_CLIENT_SECRET` | `您的 TDX Client Secret` | 從 TDX 官網取得的應用程式密鑰。 |
...
2. 檢查 `VITE_TDX_CLIENT_ID` 與 `VITE_TDX_CLIENT_SECRET` 是否正確設定（是否有大小寫錯誤或多出空白）。
