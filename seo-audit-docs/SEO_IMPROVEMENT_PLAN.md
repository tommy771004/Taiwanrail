...
- [x] 第三批 20 組回程與中段路線已建置（高鐵回程與縱貫線 8、高鐵嘉義／新竹中段 4、
      台鐵東部觀光回程 5、台鐵西部其他 3），路線頁累計 190 條、sitemap 390 個 URL。
- [x] 第四批 20 組高鐵城市對與新站已建置（臺南／嘉義縱貫線 8、嘉義／新竹中段 6、
      彰化／雲林／苗栗新站 6），路線頁累計 210 條、sitemap 430 個 URL。彰化與雲林
      在此之前完全沒有任何頁面。
- [ ] **資料驅動展開到此為止。** 剩餘 1011 組無頁面的 OD 只剩台鐵通勤短跳、台鐵
      支線小站、高鐵短跳與高鐵零星城際對四類，資料上已無法分辨誰值得做。下一批
      應改由 Search Console 的「查詢」報表決定：找有曝光但無對應頁面的字串，並針對
      CTR 偏低的既有頁面調整 title/description 措辭（長度預算已到位）。作法見
      `seo-audit-docs/ROUTE_PAGE_BACKLOG.md` 第 6 節。
